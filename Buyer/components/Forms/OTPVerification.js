...
import {
  ...
  API_FORGOT_PASSWORD_KEY,
  API_PHONE_EMAIL_OTP_VERIFICATION_KEY,
  API_PHONE_OTP_VERIFICATION_KEY,
} from '@env';
import {
  View,
  Text,
...
class ResetPasswordOTP extends Component {
  ...
  componentDidMount() {
    ...
    if (only_mobile_otp) {
      ...
      this.setState({temp_phone: phone_otp});
      this.otpPhoneTextInput[0].focus();
    } else {
      const {email_otp, phone_otp} = this.props.route?.params;
      this.setState({temp_phone: phone_otp, temp_email: email_otp});
  ...
  _socialLoginSignUp = () => {
    axios
      .post(UNIVERSAL_ENTRY_POINT_ADDRESS + API_SOCIAL_LOGIN_KEY, {
  ...
  verificationProcess = () => {
    ...
    if (otp_email.length === 4 && otp_phone.length === 4) {
      ...
      axios({
        method: 'POST',
        url:
          UNIVERSAL_ENTRY_POINT_ADDRESS + API_PHONE_EMAIL_OTP_VERIFICATION_KEY,
        data: {
          email: this.props.email,
          phone: this.props.phone,
          email_otp: otp_email,
    ...
    return (
      <View style={{height: '100%'}}>
        <ScrollView
          ...
          {this.state.temp_email || this.state.temp_phone ? (
            <View style={{alignSelf: 'center', marginTop: 80}}>
              {this.state.temp_email ? (
                <Text style={{color: '#6B23AE', fontWeight: 'bold'}}>
                  Temporary Email OTP:{' '}
                  <Text style={{color: 'black'}}>{this.state.temp_email}</Text>
                </Text>
              ) : null}
              {this.state.temp_phone ? (
                <Text style={{color: '#6B23AE', fontWeight: 'bold'}}>
                  Temporary Phone OTP:{' '}
                  <Text style={{color: 'black'}}>{this.state.temp_phone}</Text>
                </Text>
              ) : null}
            </View>
          ) : null}
            ...
            <View>
              <View
                ...
                <Text
                  ...
                  An OTP has been sent to {this.props.email} Please enter the
                  OTP in the field below to verify your Email.
                </Text>
              </View>
                  ...
                  return (
                    <View key={j} style={styles.otpinputWrap}>
                      <TextInput
                        ...
                        onKeyPress={(e) =>
                          this.focusEmailPrevious(e.nativeEvent.key, j)
                        }
                        ref={(ref) => {
                          this.otpEmailTextInput[j] = ref;
                        }}
                      />
                    </View>
                  );
          ...
          <View style={{marginTop: only_mobile_otp ? 10 : 0}}>
            <View
              ...
              <Text
                ...
                An OTP has been sent to +{this.props.phone}. Please enter the
                OTP in the field below to verify your Phone.
              </Text>
            </View>
            <View style={styles.otpBoxesContainer}>
              {otp_inputs.map((i, j) => {
                return (
                  <View key={j} style={styles.otpinputWrap}>
                    <TextInput
                      ...
                      onKeyPress={(e) =>
                        this.focusPhonePrevious(e.nativeEvent.key, j)
                      }
                      ref={(ref) => {
                        this.otpPhoneTextInput[j] = ref;
                      }}
                    />
                  </View>
                );
            ...
            <View
              style={{
                ...styles.textinputcontainermainSecond,
                flexDirection: 'row',
                alignItems: 'center',
              }}>
              ...
              <Pressable
                onPress={() =>
                  this.state.isCounterActive ? null : this._onResendOTPPress()
                }>
                <Text style={{color: '#6B23AE', fontWeight: 'bold'}}>
                  Try Again
                </Text>
              </Pressable>
              {this.state.isCounterActive ? (
                <Text style={{color: '#6B23AE', fontWeight: 'bold'}}>{` in ${
                  this.state.minuteCounter < 10
                    ? '0' + this.state.minuteCounter
                    : this.state.minuteCounter
                }:${
                  this.state.secondCounter < 10
                    ? '0' + this.state.secondCounter
          ...
          <View>
            <View style={styles.submitbtn}>
              <Ripple
                style={{marginTop: 30, rippleContainerBorderRadius: 4}}
                onPress={() => {
                  only_mobile_otp
                    ? this._phoneOtpVerificationProcess()
                    : this.verificationProcess();
        ...
        <View
          style={{
            height:
              this.state.modalVisibility && !this.state.modalLoader
                ? '100%'
                : null,
            width:
              this.state.modalVisibility && !this.state.modalLoader
                ...
                <View style={styles.modalView}>
                  ...
                  <View style={styles.button}>
                    <Ripple
                      ...
                      onPress={() => this.successfulVerificationProcess()}>
                      <LinearGradient
                        start={{x: 0, y: 0}}
                        end={{x: 1.8, y: 0}}
                        colors={['#6B23AE', '#FAD44D']}
